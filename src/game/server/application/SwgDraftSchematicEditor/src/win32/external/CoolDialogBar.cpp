...
m_clrBtnShadow(::GetSysColor(COLOR_BTNSHADOW))
{
    ...
    m_sizeHorz = CSize(100, 100);
    m_sizeVert = CSize(100, 100);
    m_sizeFloat = CSize(100, 100);
    m_bTracking = FALSE;
    m_bInRecalcNC = FALSE;
    m_cxEdge = 6;
	m_cxBorder = 3;
...
CSize CCoolDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (IsFloating())
	{
		...
		{
		case HTTOPLEFT:
			m_sizeFloat.cx = max(static_cast<int> (window_rect.right - cursor_pt.x), static_cast<int> (m_cMinWidth)) - m_cxBorder;
			m_sizeFloat.cy = max(static_cast<int> (window_rect.bottom - m_cCaptionSize - cursor_pt.y), static_cast<int> (m_cMinHeight)) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = min(static_cast<int> (cursor_pt.y), static_cast<int> (window_rect.bottom - m_cCaptionSize - m_cMinHeight)) - m_cyBorder;
			m_pDockContext->m_rectFrameDragHorz.left = min(static_cast<int> (cursor_pt.x), static_cast<int> (window_rect.right - m_cMinWidth)) - 1;
			return m_sizeFloat;
			
		case HTTOPRIGHT:
			m_sizeFloat.cx = max(static_cast<int> (cursor_pt.x - window_rect.left),
				static_cast<int> (m_cMinWidth));
			m_sizeFloat.cy = max(static_cast<int> (window_rect.bottom - m_cCaptionSize - cursor_pt.y),m_cMinHeight) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = min(static_cast<int> (cursor_pt.y), static_cast<int> (window_rect.bottom - m_cCaptionSize - m_cMinHeight)) - 
				m_cyBorder;
			return m_sizeFloat;
			
		case HTBOTTOMLEFT:
			m_sizeFloat.cx = max(static_cast<int> (window_rect.right - cursor_pt.x), static_cast<int> (m_cMinWidth)) - m_cxBorder;
			m_sizeFloat.cy = max(static_cast<int> (cursor_pt.y - window_rect.top - m_cCaptionSize), static_cast<int> (m_cMinHeight));
			m_pDockContext->m_rectFrameDragHorz.left = min(static_cast<int> (cursor_pt.x), static_cast<int> (window_rect.right - m_cMinWidth)) - 1;
			return m_sizeFloat;
			
		case HTBOTTOMRIGHT:
			m_sizeFloat.cx = max(static_cast<int> (cursor_pt.x - window_rect.left), static_cast<int> (m_cMinWidth));
			m_sizeFloat.cy = max(static_cast<int> (cursor_pt.y - window_rect.top - m_cCaptionSize), static_cast<int> (m_cMinHeight));
			return m_sizeFloat;
		}
	}
	...
	if (dwMode & LM_LENGTHY)
        return CSize(m_sizeFloat.cx,
            m_sizeFloat.cy = max(static_cast<int> (m_sizeMin.cy), nLength));
    else
        return CSize(max(static_cast<int> (m_sizeMin.cx), nLength), m_sizeFloat.cy);
}
