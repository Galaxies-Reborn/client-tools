...
void DialogConsole::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_SHAREDOLD, m_oldSharedEditCtrl);
	DDX_Control(pDX, IDC_EDIT_SHAREDNEW, m_newSharedEditCtrl);
	DDX_Control(pDX, IDC_EDIT_SERVEROLD, m_newServerEditCtrl);
	DDX_Control(pDX, IDC_EDIT_SERVERNEW, m_oldServerEditCtrl);
...
void DialogConsole::print (CString const & text)
{
...
void DialogConsole::execute (CString const & text)
{
	if (!text.IsEmpty ())
	{
		GetShellEditCtrl().SetWindowText("");
		...
		GetSelectorTabCtrl ().SetCurSel (1);
		LRESULT result;
		OnSelchangeTabSelector (0, &result);
...
void DialogConsole::setOldServerTpf (CString const & text)
{
	CString line (text);
	line.Remove ('\r');
	line.Replace ("\n", "\r\n");
	m_oldServerEditCtrl.SetWindowText (line);
}

// ----------------------------------------------------------------------

void DialogConsole::setOldSharedTpf (CString const & text)
{
	CString line (text);
	line.Remove ('\r');
	line.Replace ("\n", "\r\n");
	m_oldSharedEditCtrl.SetWindowText (line);
}

// ----------------------------------------------------------------------

void DialogConsole::setNewServerTpf (CString const & text)
{
	CString line (text);
	line.Remove ('\r');
	line.Replace ("\n", "\r\n");
	m_newServerEditCtrl.SetWindowText (line);
}

// ----------------------------------------------------------------------

void DialogConsole::setNewSharedTpf (CString const & text)
{
	CString line (text);
	line.Remove ('\r');
	line.Replace ("\n", "\r\n");
	m_newSharedEditCtrl.SetWindowText (line);
}
...
CEdit & DialogConsole::GetOldServerEditCtrl ()
{
	return m_oldServerEditCtrl;
}

// ----------------------------------------------------------------------

CEdit & DialogConsole::GetNewServerEditCtrl ()
{
	return m_newServerEditCtrl;
}

// ----------------------------------------------------------------------

CEdit & DialogConsole::GetOldSharedEditCtrl ()
{
	return m_oldSharedEditCtrl;
}

// ----------------------------------------------------------------------

CEdit & DialogConsole::GetNewSharedEditCtrl ()
{
	return m_newSharedEditCtrl;
}
...
BOOL DialogConsole::OnInitDialog() 
{
	...
	{
		DWORD dwStyle = GetWindowLong (GetShellEditCtrl().GetSafeHwnd(), GWL_STYLE);
		if (dwStyle)
		{
			dwStyle |= DS_LOCALEDIT;
			SetWindowLong (GetShellEditCtrl ().GetSafeHwnd (), GWL_STYLE, dwStyle);
		}
	}
	...
	GetSelectorTabCtrl ().InsertItem (0, "Output");
	GetSelectorTabCtrl ().InsertItem (1, "Shell");
	GetSelectorTabCtrl ().InsertItem (2, "Old server .tpf");
	GetSelectorTabCtrl ().InsertItem (3, "New server .tpf");
	GetSelectorTabCtrl ().InsertItem (4, "Old shared .tpf");
	GetSelectorTabCtrl ().InsertItem (5, "New shared .tpf");
	GetSelectorTabCtrl ().SetCurSel (0);

	m_editCtrlList [0] = &GetOutputEditCtrl ();
	m_editCtrlList [1] = &GetShellEditCtrl ();
	m_editCtrlList [2] = &GetOldServerEditCtrl ();
	m_editCtrlList [3] = &GetNewServerEditCtrl ();
	m_editCtrlList [4] = &GetOldSharedEditCtrl ();
	m_editCtrlList [5] = &GetNewSharedEditCtrl ();
	...
	for (int i = 0; i < T_COUNT; ++i)
	{
		...
		m_editCtrlList [i]->ShowWindow (i == 0 ? SW_SHOW : SW_HIDE);
	}
