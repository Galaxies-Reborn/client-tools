									 ...
									 LPCTSTR pszChars, int nOffset, int nCount)
{
	...
	if (m_nCharHeight != m_nLineHeight)
	{
		 COLORREF clr = pdc->GetBkColor();
		 pdc->FillSolidRect(rcClip.left, rcClip.top + m_nCharHeight, rcClip.right, rcClip.top + m_nLineHeight, clr);
	}
}
...
void CCrystalTextView::OnDraw(CDC *pdc)
{
	...
	rcClient.top = ((CPaintDC*)pdc)->m_ps.rcPaint.top;
	rcClient.bottom = ((CPaintDC*)pdc)->m_ps.rcPaint.bottom;
	...
	int nCurrentLine = m_nTopLine + ((CPaintDC*)pdc)->m_ps.rcPaint.top / nLineHeight;

	CRect rcLine;
	rcLine = rcClient;
	rcLine.top = nLineHeight * (((CPaintDC*)pdc)->m_ps.rcPaint.top / nLineHeight);
	rcLine.bottom = rcLine.top + nLineHeight * 2;

	CRect rcCacheMargin(0, rcLine.top, GetMarginWidth(), nLineHeight + rcLine.top);
	CRect rcCacheLine(GetMarginWidth(), rcLine.top, rcLine.Width(), nLineHeight + rcLine.top);

	while (rcLine.top < rcClient.bottom)
	{
		if (nCurrentLine < nLineCount)
		{
			DrawMargin(pdc, rcCacheMargin, nCurrentLine);
			DrawSingleLine(pdc, rcCacheLine, nCurrentLine);
		}
		else
		{
			DrawMargin(pdc, rcCacheMargin, -1);
			DrawSingleLine(pdc, rcCacheLine, -1);
		}

		nCurrentLine ++;
		rcLine.top += nLineHeight;
		rcCacheMargin.OffsetRect(0, nLineHeight);
