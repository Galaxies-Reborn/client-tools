...
{
	constexpr int cs_colorCorrectionTableSize = 256;
	constexpr int cs_colorChannelMaximum = 255;
	...
	uint8 ms_colorCorrectionTable[cs_colorCorrectionTableSize];
...
void Direct3d11_SceneTarget::setBrightnessContrastGamma(float brightness, float contrast, float gamma)
{
	...
	float const step = 1.0f / static_cast<float>(cs_colorCorrectionTableSize);
	float const oneOverGamma = (gamma != 0.0f) ? (1.0f / gamma) : 1.0f;

	float f = 0.0f;
	for (int i = 0; i < cs_colorCorrectionTableSize; ++i, f += step)
	{
		...
		int value = static_cast<int>(result * static_cast<float>(cs_colorChannelMaximum));
		if (value < 0)
			value = 0;
		if (value > cs_colorChannelMaximum)
			value = cs_colorChannelMaximum;
...
void Direct3d11_SceneTargetNamespace::releaseBuffers()
{
	...
	{
		ms_readbackBuffer->Release();
		ms_readbackBuffer = NULL;
	}
	if (ms_depthView)
	{
		ms_depthView->Release();
		ms_depthView = NULL;
	}
	if (ms_depthBuffer)
	{
		ms_depthBuffer->Release();
		ms_depthBuffer = NULL;
	}
	if (ms_resolvedResourceView)
	{
		ms_resolvedResourceView->Release();
		ms_resolvedResourceView = NULL;
	}
	if (ms_resolvedBuffer)
	{
		ms_resolvedBuffer->Release();
		ms_resolvedBuffer = NULL;
	}
	if (ms_colorResourceView)
	{
		ms_colorResourceView->Release();
		ms_colorResourceView = NULL;
	}
	if (ms_colorView)
	{
		ms_colorView->Release();
		ms_colorView = NULL;
	}
	if (ms_colorBuffer)
	{
		ms_colorBuffer->Release();
		ms_colorBuffer = NULL;
	}
}
...
void Direct3d11_SceneTargetNamespace::uploadColorCorrectionTable()
{
	...
	box.top = 0;
	box.front = 0;
	box.right = cs_colorCorrectionTableSize;
	box.bottom = 1;
	box.back = 1;
	context->UpdateSubresource(ms_colorCorrectionTexture, 0, &box, ms_colorCorrectionTable, cs_colorCorrectionTableSize, cs_colorCorrectionTableSize);
}
...
bool Direct3d11_SceneTargetNamespace::createCompositeResources()
{
	...
	tableDescription.Width = cs_colorCorrectionTableSize;
...
{
	if (ms_compositeRasterizerState)
	{
		ms_compositeRasterizerState->Release();
		ms_compositeRasterizerState = NULL;
	}
	if (ms_compositeBlendState)
	{
		ms_compositeBlendState->Release();
		ms_compositeBlendState = NULL;
	}
	if (ms_compositeDepthState)
	{
		ms_compositeDepthState->Release();
		ms_compositeDepthState = NULL;
	}
	if (ms_colorCorrectionResourceView)
	{
		ms_colorCorrectionResourceView->Release();
		ms_colorCorrectionResourceView = NULL;
	}
	if (ms_colorCorrectionTexture)
	{
		ms_colorCorrectionTexture->Release();
		ms_colorCorrectionTexture = NULL;
	}
	if (ms_pointSampler)
	{
		ms_pointSampler->Release();
		ms_pointSampler = NULL;
	}
	if (ms_compositeCorrectShader)
	{
		ms_compositeCorrectShader->Release();
		ms_compositeCorrectShader = NULL;
	}
	if (ms_compositeCopyShader)
	{
		ms_compositeCopyShader->Release();
		ms_compositeCopyShader = NULL;
	}
	if (ms_compositeVertexShader)
	{
		ms_compositeVertexShader->Release();
		ms_compositeVertexShader = NULL;
	}
}
