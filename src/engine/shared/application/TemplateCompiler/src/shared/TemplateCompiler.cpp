...
// Perforce checkOut/checkIn support removed 2026-08-29 - the tool machines
// have no p4, and the editors only ever spawn -compile/-compileeditor.
...
int main(int argc, char *argv[ ])
{
	...
	// SetupSharedRegex::install() removed for the x64 build (2026-08-29): it only
	// redirects pcre's allocator hooks into the engine memory manager, and the
	// x64 port links pcre as a DLL whose import library cannot satisfy the
	// pcre_malloc/pcre_free DATA symbols that PCRE_STATIC-compiled sharedRegex
	// references. The DLL uses its own allocator; nothing else here needs the hook.
