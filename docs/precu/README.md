...
## Core3 Full Auto Area II

Background-input protocol 193 adds `QueueFullAutoArea2` and
`FullAutoArea2WeaponStatus` at fixed IDs 302-303. The identity-bound
CDEF-carbine path proves retained Carbineer Support III ownership before an
authenticated client queues production `fullAutoArea2`. The server owns the
Patch 12 zero-time queue, 30-degree cone, RANDOM-pool damage, exact HAM costs,
`a_auto` prose, generated `fire_area` intensity animation, three defended
native state attempts, persistence observation, and reversible cleanup.

## Core3 Charge Shot II

Background-input protocol 194 adds `QueueChargeShot2` and
`ChargeShot2WeaponStatus` at fixed IDs 304-305. The identity-bound CDEF-carbine
path proves retained Carbineer Support IV ownership before an authenticated
client queues production `chargeShot2`. The server owns the Patch 12
...
## Core3 Flushing Shot One

Background-input protocol 205 adds `QueueFlushingShot1` and
`FlushingShot1WeaponStatus` at fixed IDs 326-327. The identity-bound CDEF-rifle
path uses the standard bounded repeat count and production toolbar queue. The
server owns retained Rifleman Ability I admission, Patch 12 timing, Core3
...
## Core3 Two-Handed Lunge One

Background-input protocol 210 adds `QueueMelee2hLunge1` and
`Melee2hLunge1WeaponStatus` at fixed IDs 336-337. The identity-bound
two-handed path uses the standard bounded repeat count and production toolbar
queue. The server owns retained Brawler novice admission, Patch 12 timing,
Core3 random-pool damage, `lower_posture_2hmelee_1` animation, posture-down
application and active recovery, restart persistence, and reversible cleanup.

## Core3 One-Handed Dizzy Hit One

Background-input protocol 211 adds `QueueMelee1hDizzyHit1` and
`Melee1hDizzyHit1WeaponStatus` at fixed IDs 338-339. The identity-bound
one-handed path uses the standard bounded repeat count and production toolbar
queue. The server owns the retained Brawler one-hand tier-three skill chain,
...
## Core3 Unarmed Stun One

Background-input protocol 215 adds `QueueUnarmedStun1` and
`UnarmedStun1WeaponStatus` at fixed IDs 346-347. The identity-bound unarmed
path uses the standard bounded repeat count and production toolbar queue. The
