...
# Which (pixel program, sampler stage) pairs are fed a cube map, generated by envmap_stages.py
# from the effects' own texture tags. D3D9 assembly before ps_2_0 declared no sampler dimension,
# so this is the one thing a converted pixel program cannot recover from its source -- and D3D11
# rejects a read whose declared type disagrees with the bound view, which does not render.
ENVMAP_STAGES = "envmap-stages.tsv"
...
def load_envmap_stages():
    """{pixel program: {sampler stage}} for the stages an effect feeds a cube map."""
    path = os.path.join(HERE, ENVMAP_STAGES)
    if not os.path.exists(path):
        raise SystemExit("%s is missing -- run envmap_stages.py to regenerate it. Converting "
                         "without it declares every environment stage as a 2D read, which "
                         "compiles cleanly and then does not draw." % ENVMAP_STAGES)

    stages = {}
    for line in open(path, encoding="utf-8"):
        if line.startswith("#") or not line.strip():
            continue
        name, stage = line.rstrip("\n").split("\t")[:2]
        stages.setdefault(name.replace("\\", "/").lower(), set()).add(int(stage))
    return stages
...
class Converter(object):
    def __init__(self):
        ...
        self.modulesDone = {}
        self.errors = []
        self.cubeStages = load_envmap_stages()
    ...
    def cube_stages_for(self, rel):
        return self.cubeStages.get(rel.replace("\\", "/").lower(), set())
